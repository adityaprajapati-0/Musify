...
if (Test-Path $stateFile) {
  try {
    ...
    if ($state.pid -and (Get-Process -Id $state.pid -ErrorAction SilentlyContinue)) {
      ...
      Write-Output "Stopped Pulse server PID=$($state.pid)"
      $stopped = $true
    }
  } catch {
  }
...
if (-not $stopped -and $Port -gt 0) {
  ...
  if ($pid -and (Get-Process -Id $pid -ErrorAction SilentlyContinue)) {
    ...
    Write-Output "Stopped process listening on port $Port (PID=$pid)"
    $stopped = $true
  }
}

if (-not $stopped) {
  Write-Output "No Pulse server process found to stop."
}
